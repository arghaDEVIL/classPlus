...
export default function GreetingCardPreview({ template, user }) {
    ...
    return (
        <div id="greeting-card" className={`relative w-full aspect-[2/3] overflow-hidden rounded-2xl ${cardDesign.border}`}>
            {/* Background image */}
            <img
            ...
            {/* Avatar */}
            <Avatar className="absolute top-2 left-3 w-14 h-14 ring-3 ring-teal-400 ring-offset-1 z-20">
            ...
            <div className="z-[15]">{cardDesign.textArea}</div>
        </div>
    );
}
